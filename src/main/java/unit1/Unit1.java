package unit1;

public class Unit1 {

  // Main method to test the static methods
  public static void main(String[] args) {
    // Test your methods here by calling them and printing the results
    System.out.println("Test methods here...");
  }

  /**
   * Method 1: Returns the sum of two integers.
   * TODO: Implement this method to return the sum of the two integer parameters
   * a and b.
   *
   * @param a the first integer
   * @param b the second integer
   * @return the sum of a and b
   */
  public static int addIntegers(int a, int b) {
    // Your code here
    return (a + b); // Placeholder return
  }

  /**
   * Method 2: Converts a double to an int by casting.
   * TODO: Implement this method to cast the double parameter to an int.
   *
   * @param value the double value to cast
   * @return the value cast to an int
   */
  public static int castDoubleToInt(double value) {
    // Your code here
    return (int)value; // Placeholder return
  }

  /**
   * Method 3: Checks if a boolean expression is true.
   * TODO: Implement this method to return true if the boolean parameter is
   * true, false otherwise.
   *
   * @param value the boolean value to check
   * @return true if value is true, false otherwise
   */
  public static boolean isTrue(boolean value) {
    // Your code here
    return value; // Placeholder return
  }

  /**
   * Method 4: Returns the maximum integer value minus one.
   * TODO: Implement this method to return the maximum value of an int minus
   * one.
   *
   * @return Integer.MAX_VALUE minus one
   */
  public static int maxValueMinusOne() {
    // Your code here
    return Integer.MAX_VALUE - 1; // Placeholder return
  }

  /**
   * Method 5: Returns the product of an integer and a double as a double.
   * TODO: Implement this method to return the product of the int parameter a
   * and the double parameter b.
   *
   * @param a the integer value
   * @param b the double value
   * @return the product of a and b as a double
   */
  public static double multiplyIntAndDouble(int a, double b) {
    // Your code here
    return (double)a * b; // Placeholder return
  }
}
